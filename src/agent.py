        ...
        if context_id not in self.conversations:
            ...
            if PROMPT_MODE == "minimal":
                ...
                system_content = (
                    ...
                    "- After completing one action, briefly confirm it and proceed to the next.\n"
                    "- Do NOT stop after the first action — complete ALL requested changes.\n"
                    "- Do NOT ask 'shall I proceed with the next change?' for simple sequential tasks.\n"
                    "- EXCEPTION: For booking/cancellation involving conditional payment logic "
                    "(e.g. 'use cert only if price > $400'), present the price/details and confirm before booking.\n"
                    "- Stop for user input only if you genuinely need clarification (missing data, ambiguous choice, conditional).\n"
                    ...
                    "- For eligible reservations: cancel them even if user says 'I know there's no refund, proceed anyway'.\n"
                    "- For non-eligible reservations (e.g. basic economy, no insurance, booked >24h ago): "
                    "REFUSE cancellation even if user insists. Explain the policy rule.\n\n"
                    "Modification:\n"
                    ...
                    "  If user needs a different destination: explain the modification is not possible.\n"
                    "  If the user then asks about alternatives or is willing to cancel and rebook → proactively drive cancel+rebook flow.\n"
                    "  If the user says they only want to modify (not rebook) → explain it cannot be done, offer transfer to human.\n"
