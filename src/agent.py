        ...
        if context_id not in self.conversations:
            ...
            self.conversations[context_id].append({
                ...
                "content": (
                    ...
                    "- Only use transfer_to_human_agents as a LAST RESORT when the policy explicitly requires it. "
                    "Try to resolve the request yourself first using available tools.\n"
                    "- Do NOT invent rules that aren't in the policy. Only enforce what is explicitly stated.\n"
                    "- If the policy allows an action (e.g. cancellation for business class), DO it — "
                    "don't refuse or transfer just because the flight date has passed.\n\n"
                    "## TOOL USAGE\n"
                    ...
                    "- If the user describes something without giving an ID (e.g. 'my canceled flight', "
                    "'my recent order'), look up their account and check each relevant item yourself "
                    "to find the right one. Do NOT ask the user for IDs they might not know.\n"
                    "- Only ask the user for info they would reasonably know (name, email, what they want). "
                    "Never ask for internal IDs, reservation codes, or system details.\n"
                    "- When a user gives a city name, map it to the most common airport code yourself "
                    "(e.g. New York → JFK, Chicago → ORD, Los Angeles → LAX). Do NOT ask the user for airport codes.\n"
                    "- Be efficient with steps. If you can deduce which item the user means from context, "
                    "do so instead of asking or listing all options.\n\n"
                    "## RESPONSE FORMAT\n"
